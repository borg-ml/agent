...
fn default_model_for_cross_provider_peer(provider: CodingProvider) -> Option<String> {
    match provider {
        ...
        CodingProvider::Anthropic | CodingProvider::OpenAiCompatible => None,
    }
}
    ...
    vec![
        ...
        tool(
            ...
            json!({"type":"object","properties":{
                ...
                "include_stale":{"type":"boolean","description":"Also list remote rows the directory stopped refreshing."},
                "live":{"type":"boolean","description":"Only local instances answering on their control socket right now. A peer on another host can never match this; use status for those."},
                "running":{"type":"boolean","description":"Only local instances whose owning process is still alive, reachable or not."},
                "cwd":{"type":"string","description":"Only instances launched against this exact working directory. Remote rows carry the directory their owning host reported."},
                "include_exited":{"type":"boolean","description":"Also list retired rows: local instances whose owner is gone, and instances their owning host reports as stopped."}
            },"additionalProperties":false}),
        ),
        message_tool(
            "send_message",
        ...
        tool(
            ...
            json!({
                ...
                "properties": {
                    ...
                    "coordinate_space": {"type": "string", "enum": ["desktop", "window"]},
