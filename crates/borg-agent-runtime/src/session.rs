...
async fn run_agent_session_store_kernel(
    ...
    // The goal's own status is the single stop/resume control. Escape pauses
    // the active goal and interrupts the current turn; `/goal resume` (or a
    // new objective) reactivates it. While no goal is active, automatic
    // continuation and idle background reports cannot open a provider turn.
    ...
    loop {
        ...
        if at_turn_boundary {
            ...
            // already ran. Settle again so a report that arrived while no goal
            // was active is committed to the transcript before the human
            // prompt selected below opens the next turn.
            let goal_active_after_boundary = goal
                .as_ref()
                .is_some_and(|goal| goal.status == GoalStatus::Active);
            if !goal_active_after_boundary {
                settle_inactive_team_notifications(
                    &mut journal,
                    &events,
                    session_id,
                ...
                match command {
                    ...
                    Some(HostCommand::Goal {
                        ...
                        if goal
                            ...
                            .is_some_and(|goal| goal.status == GoalStatus::Active)
                        {
                            // Explicit human resume or new objective: an active
                            // goal starts its continuation turn immediately.
                            record(
                                &mut journal,
                                &events,
                                session_id,
        ...
        loop {
            tokio::select! {
                ...
                kind = provider_events.recv(), if provider_events_open => {
                    ...
                    .await?;
                    record(&mut journal, &events, session_id, kind).await?;
                    if retry_steers && !context_compaction_in_progress && !interrupted {
                ...
                steer_result = steer_results.recv(), if !pending_steers.is_empty() => {
                    ...
                    if pending_steers[index].admission.is_accepted() {
                        ...
                        });
                        {
                            ...
                            if boundary_already_passed && !context_compaction_in_progress && !interrupted {
                                retry_pending_steers(
                                    &control_tx,
                                    &steer_result_tx,
                                    &mut pending_steers,
                ...
                command = next_host_command(&mut deferred_commands, &mut commands) => {
                    ...
                    match command {
                        ...
                        HostCommand::Prompt {
                            ...
                            if has_pending && !context_compaction_in_progress && !interrupted {
                        ...
                        HostCommand::Goal { action, .. } => {
                            ...
                            {
                                // A new objective that lands while a turn is
                                // already running steers that turn; a resume
                                // simply leaves the goal active so the next
                                // boundary continues automatically.
...
/// cannot seize the boundary from a human. Pausing the goal therefore parks the
/// agent exactly as the old stop gate did.
