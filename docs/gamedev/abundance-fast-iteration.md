...
**Priority shortlist, ranked by expected reduction in trustworthy-feedback time**

This ranking considers repeated cost and invalid work, not just worst-case
duration. Savings overlap and must not be added together. The provenance and
preflight foundation in item 2 ships with the first reusable-session pilot.

| Rank and concrete change | Expected saving, explicitly an estimate unless stated otherwise | Provenance and prevented failure | Risk and small acceptance check |
| --- | --- | --- | --- |
| **1. Reuse an isolated scene; batch compatible poses/variants.** Plugin leases one private, revision-pinned UE session; Abundance supplies reset/setup and render readiness. Reuse RiderReview's successful batch pattern. | For a compatible forest pose, retain the same 8 s settle and use the reported approximately 4 s warm capture: roughly 40–45 s saved against a 54–57 s cold run. If all 25 launches were compatible with one binary/content generation, the engine-time saving could be approximately 16–18 min including one startup/PIE setup. Actual savings depend on how many launches crossed revisions or required fresh processes. None of this estimate includes the 34.2 min gaps. | Pin source, loaded modules, content and fixture for the lease; record each pose/variant. Prevent repeated boot/setup and capture against a different branch's editor. | State contamination and memory growth. Check three matched poses cold versus warm, then repeat in reverse order after reset; require comparable images, correct identities and no material growth across repeated batches. Keep the shared main editor untouched. |
| **2. Reject invalid experiments early; publish complete capture receipts.** Make test discovery, save/world pairing, required assets, loaded revision and scene predicates part of one runner. Record actual camera motion and required test execution. | Can prevent a recurrence of the reported 650 s deterministic failure loop. Saves the launches for known-zero selectors and the entire invalid 72-frame experiment; their precise elapsed costs are not established. Cached manifest/fixture preflight target: under 2 s. | Receipt binds source snapshot including dirty changes, binary/content/save/world hashes, test identities, camera/pawn/FOV, settings, frame/time and image digest. Prevent false passes, wrong-world loops, static “motion” and unverified final-binary claims. | Overly strict preflight or incomplete runtime discovery. Feed one invalid selector, mismatched save, missing asset and stale binary; require explicit failures and zero experiment launches when knowable before launch. A valid moving recipe must report nonzero trajectory and expected executed tests. |
| **3. Coordinate peak memory and allocate a small prepared-workspace pool.** Borg atomically admits builds, resident editors and runs against the same resource ledger; wrappers request the real workload's budget. Keep source trees cheap, heavy outputs in exclusive reusable slots. | Primarily removes long-tail OOM/restart/reprovisioning costs. An avoided editor kill can save the reported approximately 43 s editor + 12 s PIE setup, plus lost work; actual recovery time varies. Existing fixed-path cache measurements suggest large cold-build savings, but they cannot be applied directly to the 376/256 s horse builds without a matched trial. The 7m58s wait cannot simply be promised away if the job truly depends on that build. | Every slot holds one immutable input/content generation with private outputs, save and ports; reservations cover actual processes, not only lock keepers. Prevent simultaneous admissions against the same headroom, disk exhaustion and cross-branch outputs. | Conservative budgets may queue too much; poor affinity may thrash caches. In an isolated test, two concurrent requests that each fit alone but not together must admit only one; cancellation/crash releases capacity once. Then verify three revisions through two slots without stale artifacts or budget breach. |
| **4. Make common visual parameters live; restart only for a requested new generation.** Extend the existing FarGrass/FarCanopy runtime-setting pattern; add explicit apply/readback for headlamp/material/pose tuning. Coalesce generation-aware restarts. | For each tuning arm that genuinely needed a leaf build, saves its reported 8–50 s build plus any reload/setup it caused. A demand-aware restart can separately remove up to the configured 45 s quiet delay after a settled build. These overlap with item 1; C++/UHT/shader structural changes still pay their necessary cost. | Store effective overrides and apply mode in the receipt; commit the winning defaults and capture again after ordinary startup. Prevent no-op setters, stale scenes and an experimental override being mistaken for the shipped result. | Some settings are construction-time or need render-state recreation. Change one known live scalar twice without UBT or process restart, verify rendered differences, restore baseline, then verify the committed choice after a fresh launch. |
| **5. Capture reproducible user moments and replay bounded action recipes.** Abundance records scene/setup; Borg schedules private-display input and capture; the plugin returns keyframes plus relevant runtime observations. | Targets repeated pose reconstruction, ad hoc scripting and investigation within inter-launch gaps. No defensible numeric saving yet. Measure time to reproduce a reported scene and number of model/tool exchanges; do not claim all 34.2 min disappears. | Attach input/event trace, source/binary/assets/save identity and observed transforms to the user's screenshot. Real input and authority postconditions preserve the player workflow; screenshots remain required for material/presentation claims. | Replay may diverge with physics/networking; observation can miss per-frame defects. Reproduce one rider interaction and one moving foliage route three times, detect the known visual defect, verify motion/input outcome, and report any divergence explicitly. |
