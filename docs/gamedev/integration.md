...
| `gamedev/bench` | `f030294` | Four deterministic simulator tests ran with `python3 -m unittest -q scripts/test_gamedev_benchmark.py`: OK (0.002 s). Owner reports modeled 6×8 naive 177.7 s/FIFO 174.9 s/Borg 56.4 s; not observed throughput. Public-CLI replay on lanes `37c5ee8` observed 3×3: 2.831 s (7 unique launches/2 joins), 6×8: 8.635 s (44/4), no conflicts/OOM; low-disk refusal exit 125. Fake processes, not Unreal or scope recovery. | run real CLI replay once lanes CLI is built; verify cleanup and observed timings separately. |
