...
Evidence for lane and service contention comes from driving the real `borg`
binary with synthetic jobs (below). Before the lane CLI existed, a deterministic
Python model of the policy (`scripts/gamedev_benchmark.py`, retrievable with
`git show 62de8686:scripts/gamedev_benchmark.py`) predicted that per-key FIFO
with RAM reservation and pending-only coalescing would cut a 6-agent × 8-job
Abundance-shaped scenario's makespan about 3× against a global lock, while
removing blind poll retries. Its inputs were Abundance `docs/BUILD_LANE.md` and
`docs/AGENT_PRODUCTIVITY_2026-09-23.md` timings plus explicit scenario
assumptions. It modeled the intended policy, not the implementation, so it was
removed once the Rust scheduler and the CLI gates below covered that policy.
